...
class IndexWorker(appContext: Context, workerParameters: WorkerParameters) : CoroutineWorker(
    ...
    override suspend fun doWork(): Result = coroutineScope {
        Timber.tag("VutIndexWorker").d("Worker has started to work!")
        if (authRepository.quickLogin()) {
            sendNotification(indexRepository.compareIndexes(this))
        }
        Result.success()
    }

    private fun sendNotification(difference: Difference?) {
